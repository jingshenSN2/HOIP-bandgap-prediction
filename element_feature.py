...
def raw_drop_duplicates():
    df = __add_formula(pd.read_csv('HOIP-30.csv', header=0))
    df = df.drop_duplicates('formula')
    df.drop(['formula'], axis=1, inplace=True)
    df.to_csv('HOIP-30_drop.csv', index=False)


def element_feature():
    df = pd.read_csv('HOIP-30_drop.csv', header=0)
    ...
    dfA1.to_csv('A_feature.csv', index=False)
    dfB1.to_csv('B_feature.csv', index=False)
    dfX1.to_csv('X_feature.csv', index=False)
